...
<ParamField body="query" param-type="string" required="true">
  This selects which records should be matched. It's the same format as our
  search endpoint.
</ParamField>

<ParamField body="num_records" param-type="integer">
  The number of records matching the query you want to be updated. If this
  parameter is not specified, all matching records will be updated.
</ParamField>
...
<ParamField body="auto_trace" param-type="boolean" required="true">
  Set this to `true` or `false` for whether or not to autotrace the property
  records matched by your `query`.
</ParamField>

<ParamField body="only_traceable" param-type="boolean">
  Boolean `true` or `false` for whether the records returned must have people
  data in the dataset.
</ParamField>

<ParamField body="view" param-type="array">
  An array of field objects to customize which fields are returned. Each object
  requires a `name` and optionally supports `flatten` and `sub_fields`. For more
  information, see [Views](https://docs.datafiniti.co/docs/property-data-views).
</ParamField>
...
<ResponseField name="total_cost" field-type="integer">
  The number of credits this request has cost you. Autotrace charges 1 property
  data credit and 1 people data credit per record.
</ResponseField>

<ResponseField name="records" field-type="array">
  The matching property records with appended people data. Each record contains
  the property fields and any traced owner contact information.
</ResponseField>
